...
const mongoURI =
  "mongodb+srv://agrisync:55e3QB2Hgn2yLTtD@cluster0.zm87w.mongodb.net/?retryWrites=true&w=majority";
...
const TouchInteraction = mongoose.model(
  "TouchInteraction",
...
class UIOptimizationModel {
  ...
  async verifyModel() {
    try {
      ...
      if (shape[1] !== 4) {
        throw new Error(
          `Invalid prediction shape: expected [n,4], got [${shape}]`
        );
      }
      testInput.dispose();
      prediction.dispose();
  ...
  async trainModel(interactions) {
    try {
      ...
      const centerX =
        touchPoints.reduce((sum, p) => sum + p.x, 0) / touchPoints.length;
      const centerY =
        touchPoints.reduce((sum, p) => sum + p.y, 0) / touchPoints.length;
      ...
      const optimalSize = {
        width: Math.max(
          Math.min(currentSize.width * baseSizeMultiplier, currentSize.width),
          20 // Minimum touch target size
        ),
        height: Math.max(
          Math.min(currentSize.height * baseSizeMultiplier, currentSize.height),
          20
        ),
      };
  ...
  async predict(metrics) {
    try {
      ...
      return [
        result[0][0],
        result[0][1],
        Math.max(result[0][2] * metrics.screenWidth, minWidth) /
          metrics.screenWidth,
        Math.max(result[0][3] * metrics.screenHeight, minHeight) /
...
app.post("/api/touch-interactions", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error saving interaction", details: error.message });
  }
});
...
app.post("/api/train", async (req, res) => {
  ...
  try {
    ...
    console.log(
      `Training model with ${successfulClicks.length} successful interactions out of ${interactions.length} total`
    );
...
app.get("/api/user-recommendations/:userID", async (req, res) => {
  ...
  try {
    ...
    if (interactions.length === 0) {
      return res
        .status(404)
        .json({ error: "No interactions found for this user" });
    }
    ...
    for (const buttonId of buttonIds) {
      const buttonInteractions = interactions.filter(
        (i) => i.buttonId === buttonId
      );
      const successfulClicks = buttonInteractions.filter((i) => !i.isMissClick);

      if (successfulClicks.length === 0) {
        recommendations[buttonId] = {
          error: "No successful clicks for this button",
        };
        continue;
      }

      // Compute basic statistics
      const avgDimensions = successfulClicks.reduce(
        (acc, click) => ({
      ...
      const missClickRate =
        (buttonInteractions.length - successfulClicks.length) /
        buttonInteractions.length;
      ...
      const finalWidth = Math.max(
        Math.round(avgDimensions.width * sizeMultiplier),
        44
      ); // Minimum size 44px
      const finalHeight = Math.max(
        Math.round(avgDimensions.height * sizeMultiplier),
        44
      );
...
app.get("/api/button-recommendations/:buttonId", async (req, res) => {
  ...
  try {
    ...
    const missClickRate =
      (interactions.length - successfulClicks.length) / interactions.length;

    // Calculate precision score
    const maxSpread = 20; // pixels
    const precisionScore =
      1 - Math.min(Math.max(xSpread, ySpread) / maxSpread, 1);
    ...
    const widthChange = Math.round(
      ((recommendedDimensions.width - avgDimensions.width) /
        avgDimensions.width) *
        100
    );
    const heightChange = Math.round(
      ((recommendedDimensions.height - avgDimensions.height) /
...
app.get('/api/miss-click-rate-by-button', async (req, res) => {
  try {
    ...
    const missClickRates = await TouchInteraction.aggregate([
      {
        $group: {
          _id: "$buttonId", // Group by buttonId
          missClicks: { $sum: { $cond: [{ $eq: ["$isMissClick", true] }, 1, 0] } }, // Count miss-clicks (isMissClick = true)
          totalInteractions: { $sum: 1 } // Count total interactions for each button
        }
      },
      {
        $project: {
          buttonId: "$_id", // Show buttonId in the result
          missClicks: 1,
          totalInteractions: 1,
          missClickRate: {
            $cond: {
              if: { $eq: ["$totalInteractions", 0] }, // Avoid division by 0
              then: 0,
              else: { $multiply: [{ $divide: ["$missClicks", "$totalInteractions"] }, 100] }
            }
          },
          _id: 0
        }
      },
      { $sort: { missClickRate: -1 } } // Sort by miss-click rate (descending)
    ]);

    res.json(missClickRates);
  } catch (error) {
...
app.get('/api/miss-click-rate', async (req, res) => {
  try {
    const totalInteractions = await TouchInteraction.countDocuments();
    const missClicks = await TouchInteraction.countDocuments({ isMissClick: true });
    const missClickRate = (missClicks / totalInteractions) * 100;
    res.json({ missClickRate: missClickRate.toFixed(2) });
  } catch (error) {
    console.error('Error fetching miss click rate:', error);
    res.status(500).json({ error: 'Error fetching miss click rate', details: error.message });
  }
});
...
app.listen(PORT, async () => {
  try {
    ...
    console.log(
      "Server will continue running without ML model. Will retry initialization on first request."
    );
  }
});
