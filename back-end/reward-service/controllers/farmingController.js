...
// New controller method to get nearby farmers
const getNearbyFarmers = async (req, res) => {
    const { lat, long, radius } = req.query;
    try {
        const result = await farmingService.getNearbyFarmers(lat, long, radius);
        logger.info('Nearby farmers retrieved');
        return res.status(result.success ? 200 : 400).json(result);
    } catch (error) {
        logger.error(`Error getting nearby farmers: ${error.message}`);
        return res.status(500).json({ success: false, message: 'Error finding nearby farmers' });
    }
};
