...
describe('Fabric', () => {
    ...
    describe('getContracts', () => {
        it('gets the Seed and qscc contracts from the network', async () => {
    ...
    describe('evatuateTransaction', () => {
        ...
        it('throws an SeedExistsError an Seed already exists error occurs', async () => {
            mockTransaction.evaluate.mockRejectedValue(
                new Error('The Seed JSCHAINCODE already exists')
            );

            await expect(async () => {
                await evatuateTransaction(mockContract, 'txn', 'arga', 'argb');
            }).rejects.toThrow(SeedExistsError);
        });

        it('throws an SeedNotFoundError if an Seed does not exist error occurs', async () => {
            mockTransaction.evaluate.mockRejectedValue(
                new Error('The Seed JSCHAINCODE does not exist')
            );

            await expect(async () => {
                await evatuateTransaction(mockContract, 'txn', 'arga', 'argb');
            }).rejects.toThrow(SeedNotFoundError);
        });
